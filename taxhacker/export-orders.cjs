...
// GST/HST small-supplier threshold (CAD). Below this, NO tax is charged.
// AE is well under $30k, so tax stays 0 unless the business registers.
...
function exportSales(){
  ...
  orders.forEach((o,i) => {
    ...
    rows.push([
      'order', i+1, o.created_at||'', escCsv(o.customer||o.customer_name||''), '', escCsv(o.note||''), escCsv(o.items.map(it=>it.title+' x'+(it.qty||1)).join(';')),
...
function exportExpenses(){
  ...
  const rows = expenses.map((o,i) => [
    ...
    money(o.subtotal), money(o.shipping||0), money(o.tax||0), money(o.total), o.currency||'CAD', o.paid_via||'cash'
  ].join(','))
